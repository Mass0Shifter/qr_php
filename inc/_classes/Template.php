...
class Template {
     ...
     */
    public function build_nav($nav_array = false, $nav_header = false, $nav_header_icons = false, $print = true) {
        ...
        // Build navigation
        $this->build_nav_array($nav_array, $nav_header, $nav_header_icons);
    ...
    private function build_nav_array($nav_array, $nav_header, $nav_header_icons) {
        foreach ($nav_array as $node) {
            ...
            $link_icon      = isset($node['icon']) && ( ! $nav_header || ($nav_header && $nav_header_icons)) ? '<i class="nav-main-link-icon ' . $node['icon'] . '"></i>' . "\n" : '';
            ...
            if ($link_type == 'heading') {
                ...
                // Set menu properties
                $li_prop        = ' class="nav-main-item' . ($sub_active ? ' open' : '') . '"';
                $link_prop      = ' class="nav-main-link' . ($link_active ? ' active' : '') . ($link_sub ? ' nav-main-link-submenu" data-toggle="submenu" aria-haspopup="true" aria-expanded="' . ($sub_active ? 'true' : 'false') . '"' : '"');
    ...
    public function get_js_personal($asset_js) {
        echo "<script src=\"$this->public_assets_folder/$asset_js\"></script>\n";
    }
    /**
